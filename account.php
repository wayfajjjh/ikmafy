...
<title>STAI YAPATA Al-Jawami </title>
...
if (@$_GET['q'] == 2)
    echo 'class="active"';
?>><a href="account.php?q=2"><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>&nbsp;My History
  ...
  $hasil_angket = mysqli_fetch_array($result);
...
<?php
    if( $hasil_angket['angket'] == 't'){
?>
<a href="account.php?q=angket"><span class="glyphicon glyphicon-list-alt" aria-hidden="true"></span>&nbsp; Angket
</a></li>
<?php
}
?>
...
?>
<?php
$endquiz = isset($_GET['endquiz']) ? $_GET['endquiz'] : "";
...
<center><h1 class="title" style="color:#660033">Result</h1><center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
        echo '<tr style="color:darkblue"><td style="vertical-align:middle">Total Soal</td><td style="vertical-align:middle">' . $total . '</td></tr>
    ...
    <tr style="color:darkblue"><td style="vertical-align:middle">Nilai&nbsp;<span class="glyphicon glyphicon-star" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $s . '</td></tr>';
        ...
        while ($row = mysqli_fetch_array($q)) {
            $s = $row['score'];
            echo '<tr style="color:#990000"><td style="vertical-align:middle">Nilai Keseluruhan&nbsp;<span class="glyphicon glyphicon-stats" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $s . '</td></tr>';
        }
...
echo "Anda mengklik";
if(count($pilihan) < 1){
  echo "Anda Belum Jawab Soal Satupun";
}else{
  echo "Anda Telah Memilih";



  foreach ($pilihan as $key => $hasil) {
    $q = mysqli_query($con, "SELECT * FROM angket where id =$key ");
    $nilai = mysqli_fetch_array($q);
    $nilai_yes = $nilai['yes'];
    $nilai_no = $nilai['no'];
    ...
    if($hasil=='yes'){
      //echo "yeah";
      $nilai_yes++;
      $yes = mysqli_query($con, "UPDATE angket SET yes=$nilai_yes where id=$key");

    }else{
      $nilai_no++;
      $no = mysqli_query($con, "UPDATE angket SET no=$nilai_no where id=$key");
...
$udah = mysqli_query($con, "UPDATE user SET angket='y' where username ='$username'");
echo "<script language=javascript> alert('Terimakasih atas Survey nya');document.location.href='account.php?q=1'  </script>";
...
$q = mysqli_query($con, "SELECT * FROM angket ");
$no = 0;
                while ($row = mysqli_fetch_array($q)) {
        ...
        ?>
            <tr>
                ...
                <td><input required type="radio" name=<?php echo "pilihan[". $row['id']."]" ?> value="yes"/></td>
                <td><input required type="radio" name=<?php echo "pilihan[". $row['id']."]" ?> value="no"/></td>
            </tr>
        ...
        <input name="hasil" type="submit"  value="Input"  style="  width: 15%;
...
<span href="#" data-target="#login" style="color:lightyellow"><br><br></span></div>
